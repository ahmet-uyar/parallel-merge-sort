...
## Constructing Complete Binary Thread Trees
Parallel merge sort algorithm in Java library always construct a full binary thread tree recursively. This is usually no problem since many multicore CPUs have 2, 4, 8, 16, etc. cores. The number of cores is commonly a power of two. However, this is not always the case. There are CPUs with 6, 12, 24 cores also. When a system has for example 12 cores, parallel merge sort in Java library constructs a full binary thread tree with 16 leaf nodes. That means, the elements to be sorted is divided by 16 and 16 threads perform initial sorting. This produces a less than optimum result. Since there are more sub arrays to sort than the number of cores. Ideally each core should get one sub array to sort iniatially. 

In my iterative implementation of MergeSortWithForkJoinSTM2, I construct not a full binary thread tree but rather a complete binary thread tree. If there are 12 cores in a system, then the binary thread tree has 12 leafs. Therefore the elements to be sorted is divided by 12 and each core gets exactly one sub array to sort before merge operations start. 
